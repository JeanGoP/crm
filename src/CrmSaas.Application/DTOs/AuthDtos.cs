...
public sealed record UserDto(Guid Id, string FullName, string Email, IReadOnlyCollection<string> Roles, Guid CompanyId, Guid? SalesPointId, string? SalesPointName);
public sealed record CreateUserDto(string FullName, string Email, string Password, Guid CompanyId, Guid? SalesPointId, IReadOnlyCollection<string> Roles);
