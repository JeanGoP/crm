...
const emptyCreditApplication = {
  ...
  coDebtorReference1Name: '', coDebtorReference1Mobile: '', coDebtorReference1Relationship: '', coDebtorReference2Name: '', coDebtorReference2Mobile: '', coDebtorReference2Relationship: '',
  status: 1, notes: ''
};
...
function QuotesPage() {
  ...
  const save = async (payload: typeof emptyQuote) => {
    ...
    const quoteItems = (payload.items?.length ? payload.items : [{ ...emptyQuoteItem, productId: payload.productId, productPrice: 0, downPayment: payload.downPayment, extraPayment: 0, insurance: payload.insurance, administrativeFees: payload.administrativeFees, termMonths: payload.termMonths, monthlyInterestRate: payload.monthlyInterestRate }])
      ...
      .map((sourceItem) => {
        const item = bundle ? { ...sourceItem, downPayment: 0, extraPayment: 0, initialPaymentSchedule: [], terms: globalPayment.terms, termMonths: globalPayment.terms[0], monthlyInterestRate: globalPayment.monthlyInterestRate } : sourceItem;
        const product = products.find((candidate) => candidate.id === item.productId);
        const chargeTotals = quoteChargeTotals(item, activeChargeConcepts, product);
        if (!payload.isCash && (!item.terms.length || item.terms.some(term => !Number.isInteger(term) || term < 1 || term > quoteTermLimit(product?.category))))
        ...
        return {
          ...
          terms: payload.isCash ? null : item.terms,
          termMonths: payload.isCash ? 0 : item.terms[0],
    ...
    const body = {
      ...
      bundlePayment: bundle && !payload.isCash ? {
        ...
        terms: globalPayment.terms,
        termMonths: globalPayment.terms[0],
...
function CreditApplicationsPage() {
  ...
  const save = async (payload: typeof emptyCreditApplication) => {
    ...
    const body = {
      ...
      coDebtorReference1Name: payload.coDebtorReference1Name || null,
      coDebtorReference1Mobile: payload.coDebtorReference1Mobile || null,
      coDebtorReference1Relationship: payload.coDebtorReference1Relationship || null,
      coDebtorReference2Name: payload.coDebtorReference2Name || null,
      coDebtorReference2Mobile: payload.coDebtorReference2Mobile || null,
      coDebtorReference2Relationship: payload.coDebtorReference2Relationship || null,
      status: Number(payload.status),
      notes: payload.notes || null
    };
    const { data } = form.item
...
function SettingsPage() {
  ...
  const saveSalesPoint = async (payload: typeof emptySalesPoint) => {
    const rates = payload.rates.map((rate) => ({
      id: rate.id || null,
      name: rate.name,
      factorMonthlyRate: Number(rate.factorMonthlyRate),
      maxTermMonths: Number(rate.maxTermMonths),
      active: Boolean(rate.active)
    }));
    const defaultRate = rates.find((rate) => rate.active) ?? rates[0];
    const body = {
      name: payload.name,
      code: payload.code,
      city: payload.city,
      ...
      factorMonthlyRate: Number(defaultRate?.factorMonthlyRate ?? payload.factorMonthlyRate),
      maxTermMonths: Number(defaultRate?.maxTermMonths ?? payload.maxTermMonths),
  ...
  return <Stack spacing={3}>
    <Header title="Configuracion" onRefresh={() => { reloadCompanies(); reloadUsers(); reloadProductCategories(); reloadFinancialSettings(); reloadQuoteChargeConcepts(); reloadSalesPoints();  reloadPromotions(); }} />
      ...
      <Stack spacing={2}>
        ...
        <EntityTable
          ...
          rows={salesPoints.map((point) => [
            ...
            <Stack spacing={.4}>{point.rates?.filter((rate) => rate.active).map((rate) => <Chip key={rate.id} size="small" label={`${rate.name}: ${rate.factorMonthlyRate}%`} />)}</Stack>,
            <Box>
...
function SalesPointDialog({ form, onClose, onSave }: DialogProps<SalesPoint, typeof emptySalesPoint>) {
  ...
  const initial = form.item ? {
    ...
    rates: form.item.rates?.length
      ? form.item.rates.map((rate) => ({ id: rate.id, name: rate.name, factorMonthlyRate: rate.factorMonthlyRate, maxTermMonths: rate.maxTermMonths, active: rate.active }))
      : [{ id: '', name: 'Tasa general', factorMonthlyRate: form.item.factorMonthlyRate, maxTermMonths: form.item.maxTermMonths, active: true }],
    active: form.item.active
  } : emptySalesPoint;
  ...
  return <FormDialog title={form.item ? 'Editar sede' : 'Nueva sede'} open={form.open} initial={initial} onClose={onClose} onSave={onSave} maxWidth="md">
    {(v, set) => {
      ...
      const rates = v.rates?.length ? v.rates : [emptySalesPointRate];
      const updateRate = (index: number, patch: Partial<typeof emptySalesPointRate>) => {
        set({ rates: rates.map((rate, rateIndex) => rateIndex === index ? { ...rate, ...patch } : rate) });
      };
      ...
      <Grid container spacing={1.5}>
        <Grid item xs={12}><TextField fullWidth required label="Marca principal" value={v.mainBrand} onChange={(e) => set({ mainBrand: e.target.value })} /></Grid>
      </Grid>
      <Paper variant="outlined" sx={{ p: 1.5, bgcolor: '#fbfdff' }}>
        <Stack spacing={1.25}>
          <Stack direction={{ xs: 'column', sm: 'row' }} justifyContent="space-between" alignItems={{ xs: 'stretch', sm: 'center' }} gap={1}>
            <Box><Typography variant="subtitle2" fontWeight={900} color="primary">Tasas disponibles en esta sede</Typography><Typography variant="caption" color="text.secondary">El asesor las seleccionara por nombre al crear la cotizacion.</Typography></Box>
            <Button type="button" variant="outlined" startIcon={<Add />} onClick={() => set({ rates: [...rates, { ...emptySalesPointRate, id: '', name: '' }] })}>Agregar tasa</Button>
          </Stack>
          {rates.map((rate, index) => <Box key={index} sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', md: 'minmax(180px, 1fr) 150px 150px 120px 44px' }, gap: 1, alignItems: 'center' }}>
            <TextField required label="Nombre de la tasa" value={rate.name} onChange={(e) => updateRate(index, { name: e.target.value })} />
            <TextField required type="number" label="Factor mensual (%)" value={rate.factorMonthlyRate} onChange={(e) => updateRate(index, { factorMonthlyRate: Number(e.target.value) })} />
            <TextField required type="number" label="Plazo maximo" value={rate.maxTermMonths} onChange={(e) => updateRate(index, { maxTermMonths: Number(e.target.value) })} />
            <FormControlLabel control={<Checkbox checked={rate.active} onChange={(e) => updateRate(index, { active: e.target.checked })} />} label="Activa" />
            <IconButton color="error" disabled={rates.length === 1} onClick={() => set({ rates: rates.filter((_, rateIndex) => rateIndex !== index) })}><Delete fontSize="small" /></IconButton>
          </Box>)}
        </Stack>
      </Paper>
      <Grid container spacing={1.5}>
...
function QuoteDialog({ form, products, productCategories, quoteChargeConcepts, salesPoints, canChooseSalesPoint, onClose, onSave }: DialogProps<Quote, typeof emptyQuote> & { products: Product[]; productCategories: ProductCategory[]; quoteChargeConcepts: QuoteChargeConcept[]; salesPoints: QuoteSalesPoint[]; canChooseSalesPoint: boolean }) {
  ...
  const initial = { ...emptyQuote, salesPointId: initialSalesPoint?.id ?? '', salesPointRateId: initialSalesPoint?.rates[0]?.id ?? '', productId: initialItem.productId, items: [initialItem] };
            ...
            return <Paper key={index} variant="outlined" sx={{ p: 2, bgcolor: '#f8fafc' }}>
              <Stack spacing={1.5}>
                ...
                <Box sx={{
                  ...
                  {!v.isCash && !isBundleQuote && <QuoteTermField label="Plazos a cotizar" values={item.terms} maximum={quoteTermLimit(selectedProduct?.category)} onChange={terms => updateItem(index, { terms, termMonths: terms[0] || 0 })} />}
...
function QuoteSimulationPreview({ value, selectedProduct, salesPointId, salesPointRateId, compact = false, isCash = false, bundleItems }: { bundleItems?: typeof emptyQuoteItem[]; isCash?: boolean; value: typeof emptyQuoteItem; selectedProduct?: Product; salesPointId?: string; salesPointRateId?: string; compact?: boolean }) {
  ...
  const termsRequest = JSON.stringify(value.terms);
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => {
      ...
      api.post<QuoteSimulationResult>('/api/quotes/simulate', {
        isCash,
        terms: isCash ? null : JSON.parse(termsRequest),
  ...
  }, [termsRequest, bundleRequest, isCash, selectedProduct?.id, productPrice, completeDownPayment, value.insurance, value.administrativeFees, value.termMonths, value.monthlyInterestRate, salesPointId, salesPointRateId]);
  ...
  const preview = (isCash === (simulation?.creditType === 'Contado') ? simulation : undefined) ?? {
    ...
    salesPointRateId: salesPointRateId || undefined,
    salesPointRateName: undefined,
  ...
  if (!isCash && !simulation) return <Paper variant="outlined" sx={{ p: 2 }}>{error ? <Alert severity="warning">{error}</Alert> : <><LinearProgress /><Typography>Calculando alternativas...</Typography></>}</Paper>;
...
function CreditApplicationDialog({ form, customers, products, quotes, onClose, onSave }: DialogProps<CreditApplication, typeof emptyCreditApplication> & { customers: Customer[]; products: Product[]; quotes: Quote[]; }) {
  ...
  const initial = form.item ? {
    ...
    coDebtorReference1Name: form.item.coDebtorReference1Name ?? '',
    coDebtorReference1Mobile: form.item.coDebtorReference1Mobile ?? '',
    coDebtorReference1Relationship: form.item.coDebtorReference1Relationship ?? '',
    coDebtorReference2Name: form.item.coDebtorReference2Name ?? '',
    coDebtorReference2Mobile: form.item.coDebtorReference2Mobile ?? '',
...
function QuoteTermField({ label, values, maximum, onChange }: { label: string; values: number[]; maximum: number; onChange: (values: number[]) => void }) {
  const [custom, setCustom] = useState('');
  const validCustom = Number.isInteger(Number(custom)) && Number(custom) >= 1 && Number(custom) <= maximum;
  const change = (term: number) => onChange(updateQuoteTerms(values, term, maximum));
  const addCustom = () => {
    if (!validCustom) return;
    onChange(updateQuoteTerms(values, Number(custom), maximum, false));
    setCustom('');
  };
  return <Box sx={{ gridColumn: '1 / -1', minWidth: 0 }}>
    <Typography fontWeight={800}>{label}</Typography>
    <Typography variant="caption" color="text.secondary">Marca uno o varios. Hasta {maximum} cuotas.</Typography>
    <Stack direction="row" gap={1} flexWrap="wrap" sx={{ mt: 1 }}>
      {[6, 12, 18, 24, 36, 40].filter(term => term <= maximum).map(term => <Button key={term} type="button" size="small"
        aria-pressed={values.includes(term)} variant={values.includes(term) ? 'contained' : 'outlined'} onClick={() => change(term)}>{term} cuotas</Button>)}
    </Stack>
    <Stack direction="row" gap={1} sx={{ mt: 1 }} alignItems="center">
      <TextField size="small" type="number" label="Otro plazo" value={custom} onChange={e => setCustom(e.target.value)}
        onKeyDown={e => { if (e.key === 'Enter') { e.preventDefault(); addCustom(); } }}
        inputProps={{ min: 1, max: maximum, step: 1 }} sx={{ width: 130 }} />
      <Button type="button" onClick={addCustom} disabled={!validCustom}>Agregar plazo</Button>
    </Stack>
    <Stack direction="row" gap={.75} flexWrap="wrap" sx={{ mt: 1 }}>
      {values.map(term => <Chip key={term} label={`${term} cuotas`} color={term > maximum || term < 1 ? 'error' : 'primary'} variant="outlined"
        onDelete={() => change(term)} />)}
    </Stack>
    {(!values.length || values.some(term => term > maximum || term < 1)) && <Typography color="error" variant="caption">Selecciona al menos un plazo válido entre 1 y {maximum}.</Typography>}
  </Box>;
}
